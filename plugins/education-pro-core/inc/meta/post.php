...
function tx_post_add_meta_box() {
	...
	foreach ( $screens as $screen ) {
		...
		add_meta_box(
			'tx_post_meta_box_id',
			esc_html__( 'Video Option', 'education-pro-core' ),
...
function tx_post_plugin_meta_box_callback( $post ) {
	...
	esc_html_e( 'Please enter Youtube video URL. You need to choose post format as Video.', 'education-pro-core' );
	echo '</label> ';
	echo '<br><br>';
	echo '<input placeholder="Example: https://youtu.be/8fLNkMZYlSw" type="text" id="tx_post_new_field" name="tx_post_new_field" value="' . esc_attr( $post_link ) . '" size="60" />';
	echo '<br>';
