...
/* ---------------------------------------------------------
   Brochure meta boxes for Classes post type
------------------------------------------------------------ */
// Add a meta box
add_action( 'admin_menu', 'tx_classes_meta_box_add' );
function tx_classes_meta_box_add() {
    add_meta_box(
        'brochure',
        esc_html__( 'Brochure Details', 'education-pro-core' ),
        'tx_brochure_details',
        'class',
        'normal',
        'high' 
...
function tx_brochure_details( $post ) {
    ...
    <table class="tx_brochure_details brochure-title">
        <tbody>
            <tr>
                <th><?php esc_html_e('Brochure Title','education-pro-core'); ?></th>
                <td><input type="text" name="brochure_title" id="brochure_title" value="<?php echo esc_attr($brochure_title); ?>" placeholder="<?php echo esc_html__('Please enter title here ex: Our Brochure etc','education-pro-core') ?>" size="78" /></td>
            </tr>
        </tbody>
    ...
    <table class="tx_brochure_details brochure-desc">
        <tbody>
            <tr>
                <th><?php esc_html_e('Brochure Description','education-pro-core'); ?></th>
                <td><textarea name="brochure_desc" id="brochure_desc" placeholder="<?php echo esc_html__('Please enter description here','education-pro-core') ?>" cols="73" ><?php echo esc_html($brochure_desc); ?></textarea></td>
    ...
    <div class="tx_brochure_details brochure-image">
        ...
        <?php endif; ?>
        <p class="brochure_img_msg"><?php echo esc_html__('Please click Upload Image button to upload Brochure image.', 'education-pro-core'); ?></p>
        <a href="#" class="brochure_img button"><?php echo esc_html__('Upload Image', 'education-pro-core'); ?></a>
        <input type="hidden" name="brochure_img" id="brochure_img" value="<?php echo esc_attr($brochure_img); ?>" />
        <a href="#" class="brochure_img_remove button"><?php echo esc_html__('Remove Image', 'education-pro-core'); ?></a>
    </div><!-- /.brochure-image -->
    ...
    <div class="tx_brochure_details brochure-file">
        ...
        <?php endif; ?>
        <p class="brochure_file_msg"><?php echo esc_html__('Please click Upload File button to upload Brochure file', 'education-pro-core'); ?></p>
        <a href="#" class="brochure_file button"><?php echo esc_html__('Upload File', 'education-pro-core'); ?></a>
        <input type="hidden" name="brochure_file" id="brochure_file" value="<?php echo esc_attr($brochure_file); ?>" />
        <a href="#" class="brochure_file_remove button"><?php echo esc_html__('Remove File','education-pro-core'); ?></a>
    </div><!-- /.brochure-file -->
    ...
    <table class="tx_brochure_details form-shortcode">
        <tbody>
            <tr>
                <th><?php esc_html_e('Form Shortcode','education-pro-core'); ?></th>
                <td><input type="text" name="form_shortcode" id="tx_form_shortcode" value="<?php echo esc_attr($form_shortcode); ?>" placeholder="<?php echo esc_html__('Please enter shortcode here ex: Contact Form 7 shortcode etc','education-pro-core') ?>" size="75" /></td>
            </tr>
        </tbody>
...
function tx_brochure_enqueue_scripts( $brochure ) {
    ...
    if( in_array($brochure, array('post.php', 'post-new.php') ) ) {
        ...
        if( is_object( $screen ) && $cpt == $screen->post_type ) {
            wp_enqueue_script( 'brochure', TX_PLUGIN_URL . '/assets/js/brochure.min.js', array('jquery'), TX_PLUGIN_VERSION, false );
        }
    }
}
    ...
    global $post;
     ...
     if(!empty($post))
     {
        $pageTemplate = get_post_meta($post->ID, '_wp_page_template', true);
         ...
         if($pageTemplate == 'templates/classes.php' ) // display on services template only
         {
            add_meta_box(
                'tx_services_settings', // $id
                esc_html__('Portfolio Settings','education-pro-core'), // $title
...
?>
  ...
  <div class="tx-portfolio-settings">
    ...
    <div>
        <p for="display"><?php echo esc_html_e('Display Type','education-pro-core'); ?></p>
        <select name="display" id="display">
          <option value="grid" <?php selected( $display, 'grid' ); ?>><?php echo esc_html_e('Grid','education-pro-core'); ?></option>
          <option value="overlay" <?php selected( $display, 'overlay' ); ?>><?php echo esc_html_e('Overlay','education-pro-core'); ?></option>
        </select>
    </div>
    <div>
        <p for="serv_category"><?php echo esc_html_e('Category(For Grid only)','education-pro-core'); ?></p>
        <select name="serv_category" id="serv_category">
          <option value="show" <?php selected( $serv_category, 'show' ); ?>><?php echo esc_html_e('Show','education-pro-core'); ?></option>
          <option value="hide" <?php selected( $serv_category, 'hide' ); ?>><?php echo esc_html_e('Hide','education-pro-core'); ?></option>
        </select>
    </div>
    <div>
        <p for="title"><?php echo esc_html_e('Title','education-pro-core'); ?></p>
        <select name="title" id="title">
          <option value="show" <?php selected( $title, 'show' ); ?>><?php echo esc_html_e('Show','education-pro-core'); ?></option>
          <option value="hide" <?php selected( $title, 'hide' ); ?>><?php echo esc_html_e('Hide','education-pro-core'); ?></option>
        </select>
    </div>
    <div>
        <p for="desc"><?php echo esc_html_e('Excerpt','education-pro-core'); ?></p>
        <select name="desc" id="desc">
          <option value="show" <?php selected( $desc, 'show' ); ?>><?php echo esc_html_e('Show','education-pro-core'); ?></option>
          <option value="hide" <?php selected( $desc, 'hide' ); ?>><?php echo esc_html_e('Hide','education-pro-core'); ?></option>
        </select>
    </div>
    <div>
        <p for="link"><?php echo esc_html_e('Link(For Overlay only)','education-pro-core'); ?></p>
        <select name="link" id="link">
          <option value="show" <?php selected( $link, 'show' ); ?>><?php echo esc_html_e('Show','education-pro-core'); ?></option>
          <option value="hide" <?php selected( $link, 'hide' ); ?>><?php echo esc_html_e('Hide','education-pro-core'); ?></option>
        </select>
    </div>
    <div>
        <p for="item_per_page"><?php echo esc_html_e('Item per page','education-pro-core'); ?></p>
        <input type="text" name="item_per_page" id="item_per_page" value="<?php echo $item_per_page; ?>" placeholder="<?php echo esc_html_e('Default 9. Enter -1 to show all','education-pro-core'); ?>" size="27" />
