...
<div class="lp-courses-bar <?php echo esc_attr( $active ); ?>">
	<form class="search-courses" method="get" action="<?php echo esc_url( learn_press_get_page_link( 'courses' ) ); ?>">
		...
		<input type="text" placeholder="<?php esc_attr_e( 'Search courses...', 'education-pro' ); ?>" name="c_search" value="<?php echo esc_attr( $s ); ?>">
	...
	<div class="switch-layout">
		<?php foreach ( $layouts as $layout => $value ) : ?>
			...
			<label class="switch-btn <?php echo $layout; ?>" title="<?php echo sprintf( esc_attr__( 'Switch to %s', 'education-pro' ), $layout ); ?>" for="lp-switch-layout-btn-<?php echo esc_attr( $layout ); ?>"></label>
		<?php endforeach; ?>
	</div>
</div>
