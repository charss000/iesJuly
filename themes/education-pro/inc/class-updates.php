<?php
if(!class_exists("EducationProBase")) {
	class EducationProBase {
    	public $key = "1B300826833ABBD5";
    	private $product_id = "1";
    	private $product_base = "education-pro";
    	private $server_host = "https://themes.network/wp-json/licensor/";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_education-pro_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=education-pro_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
		...
		function handleServerRequest(){
			...
			switch ($type){
				case "rl": //remove license
					...
					$obj->status=true;
					echo esc_attr($this->encryptObj($obj));
					return;
