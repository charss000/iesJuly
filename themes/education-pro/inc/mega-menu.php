...
function start_el( &$output, $item, $depth = 0, $args = array(), $current_object_id = 0 ) {
    ...
    if ( ! empty( $item->_invalid ) ) {
        ...
        $title = sprintf( __( '%s (Invalid)', 'education-pro'), $item->title );
    } elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
        $classes[] = 'pending';
        /* translators: %s: title of menu item in draft status */
        $title = sprintf( __('%s (Pending)', 'education-pro'), $item->title );
    }
            ...
            <dt class="menu-item-handle">
                ...
                <span class="item-controls">
                    ...
                    <span class="item-order hide-if-js">
                        ...
                        ?>" class="item-move-up"><abbr title="<?php esc_attr_e('Move up','education-pro'); ?>">&#8593;</abbr></a>
                        |
                        <a href="<?php
                            echo wp_nonce_url(
                                add_query_arg(
                        ...
                        ?>" class="item-move-down"><abbr title="<?php esc_attr_e('Move down','education-pro'); ?>">&#8595;</abbr></a>
                    </span>
                    <a class="item-edit" id="edit-<?php echo esc_attr( $item_id ); ?>" title="<?php esc_attr_e('Edit Menu Item','education-pro'); ?>" href="<?php
                        echo ( isset( $_GET['edit-menu-item'] ) && $item_id == $_GET['edit-menu-item'] ) ? admin_url( 'nav-menus.php' ) : add_query_arg( 'edit-menu-item', $item_id, remove_query_arg( $removed_args, admin_url( 'nav-menus.php#menu-item-settings-' . $item_id ) ) );
                    ?>"><?php esc_html_e( 'Edit Menu Item' ,'education-pro'); ?></a>
                </span>
            </dt>
        </dl>
        ...
        <div class="menu-item-settings" id="menu-item-settings-<?php echo esc_attr( $item_id ); ?>">
            <?php if( 'custom' == $item->type ) : ?>
                <p class="field-url description description-wide">
                    <label for="edit-menu-item-url-<?php echo esc_attr( $item_id ); ?>">
                        <?php esc_html_e( 'URL' ,'education-pro'); ?><br />
            ...
            <p class="description description-thin">
                <label for="edit-menu-item-title-<?php echo esc_attr( $item_id ); ?>">
                    <?php esc_html_e( 'Navigation Label' ,'education-pro'); ?><br />
                    <input type="text" id="edit-menu-item-title-<?php echo esc_attr( $item_id ); ?>" class="widefat edit-menu-item-title" name="menu-item-title[<?php echo esc_attr( $item_id ); ?>]" value="<?php echo esc_attr( $item->title ); ?>" />
                </label>
            </p>
            <p class="description description-thin">
                <label for="edit-menu-item-attr-title-<?php echo esc_attr( $item_id ); ?>">
                    <?php esc_html_e( 'Title Attribute','education-pro' ); ?><br />
            ...
            <p class="field-link-target description">
                <label for="edit-menu-item-target-<?php echo esc_attr( $item_id ); ?>">
                    ...
                    <?php esc_html_e( 'Open link in a new window/tab' ,'education-pro'); ?>
                </label>
            </p>
            <p class="field-css-classes description description-thin">
                <label for="edit-menu-item-classes-<?php echo esc_attr( $item_id ); ?>">
                    <?php esc_html_e( 'CSS Classes (optional)' ,'education-pro'); ?><br />
                    <input type="text" id="edit-menu-item-classes-<?php echo esc_attr( $item_id ); ?>" class="widefat code edit-menu-item-classes" name="menu-item-classes[<?php echo esc_attr( $item_id ); ?>]" value="<?php echo esc_attr( implode(' ', $item->classes ) ); ?>" />
                </label>
            </p>
            <p class="field-xfn description description-thin">
                <label for="edit-menu-item-xfn-<?php echo esc_attr( $item_id ); ?>">
                    <?php esc_html_e( 'Link Relationship (XFN)' ,'education-pro'); ?><br />
                    <input type="text" id="edit-menu-item-xfn-<?php echo esc_attr( $item_id ); ?>" class="widefat code edit-menu-item-xfn" name="menu-item-xfn[<?php echo esc_attr( $item_id ); ?>]" value="<?php echo esc_attr( $item->xfn ); ?>" />
                </label>
            </p>
            <p class="field-description description description-wide">
                <label for="edit-menu-item-description-<?php echo esc_attr( $item_id ); ?>">
                    <?php esc_html_e( 'Description' ,'education-pro'); ?><br />
                    ...
                    <span class="description"><?php esc_html_e('The description will be displayed in the menu if the current theme supports it.','education-pro'); ?></span>
                </label>
            </p>
            <?php
            /*
             ...
             */
			if ( ! $depth ) {
				$title = esc_html__('Submenu Type','education-pro');
				...
				<p class="description description-thin">
					...
					<label for="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>">
						<select id="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>" class=" <?php echo esc_attr( $key ); ?>" name="<?php echo esc_attr( $key . "[" . $item_id . "]" ); ?>">
							<option value="standard" <?php echo ( esc_attr($value) == 'standard' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( 'Standard Dropdown', 'education-pro' ); ?></option>
							<option value="columns1" <?php echo ( esc_attr($value) == 'columns1' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( '1 column Dropdown', 'education-pro' ); ?></option>
                            <option value="columns2" <?php echo ( esc_attr($value) == 'columns2' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( '2 columns Dropdown', 'education-pro' ); ?></option>
							<option value="columns3" <?php echo ( esc_attr($value) == 'columns3' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( '3 columns Dropdown', 'education-pro' ); ?></option>
							<option value="columns4" <?php echo ( esc_attr($value) == 'columns4' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( '4 columns Dropdown', 'education-pro' ); ?></option>
						</select>
					</label>
				</p>
            <?php } ?>
		    ...
		    <?php if($depth) {
    			$title = esc_html__('Widget Area','education-pro');
    			...
    			<p class="description description-thin"<?php echo esc_attr( $style ); ?>>
    				...
    				<label for="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>">
    					<select id="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>" class=" <?php echo esc_attr( $key ); ?>" name="<?php echo esc_attr( $key . "[" . $item_id . "]" ); ?>">
    						<option value="" <?php echo ( esc_attr($value) == '' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( 'Select Widget Area', 'education-pro' ); ?></option>
    						<?php
			...
			<?php if($depth) {
    			$title = esc_html__('Hide Link?', 'education-pro');
    			...
    			<p class="description description-thin">
    				...
    				<label for="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>">
    					<select id="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>" class=" <?php echo esc_attr( $key ); ?>" name="<?php echo esc_attr( $key . "[" . $item_id . "]" ); ?>">
    						<option value="0" <?php echo ( esc_attr($value) == '0' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( 'No', 'education-pro' ); ?></option>
    						<option value="1" <?php echo ( esc_attr($value) == '1' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( 'Yes', 'education-pro' ); ?></option>
    					</select>
    				</label>
    			</p>
    		<?php } ?>
            <?php     
                $title = esc_html__('Icon(EX: fa fa-home)','education-pro');
                $LineAwesome = esc_html__('Icon(EX: la la-home)','education-pro');
                $key = "menu-item-menu_icon";
                $value = $item->menu_icon;
                ?>
                <p class="description description-thin">
                    <label for="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>">
                        <?php echo esc_html( $title ); ?> <a href="<?php echo esc_url('https://fontawesome.com/v4.7.0/icons/'); ?>" target="_blank"><?php esc_html_e('FontAwesome Icon URL', 'education-pro'); ?></a><br />
                        <?php echo esc_html( $LineAwesome ); ?> <a href="<?php echo esc_url('https://icons8.com/line-awesome'); ?>" target="_blank"><?php esc_html_e('LineAwesome Icon URL', 'education-pro'); ?></a><br />
            ...
            <?php
    			if ( ! $depth ) {
                $title = esc_html__('DropDown Background Image','education-pro');
                ...
                <p class="description description-wide">
                    <label for="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>">
                        <?php echo esc_html( $title ); ?><br />
                        <input type="text" value="<?php echo esc_attr( $value ); ?>" id="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>" class=" <?php echo esc_attr( $key ); ?>" name="<?php echo esc_attr( $key . "[" . $item_id . "]" ); ?>" />
                        <button id="browse-edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>" class="set_custom_images button button-secondary submit-add-to-menu"><?php esc_html_e( 'Browse Image', 'education-pro' ); ?></button>
    					<a class="button btn_clear button-primary" href="javascript: void(0);"><?php echo esc_html__('Clear','education-pro');?></a>
                    </label>
                </p>
            ...
            <div class="menu-item-actions description-wide submitbox">
                <?php if( 'custom' != $item->type && $original_title !== false ) : ?>
                    <p class="link-to-original">
                        <?php printf( __('Original: %s', 'education-pro'), '<a href="' . esc_attr( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
                    </p>
                <?php endif; ?>
                ...
                ); ?>"><?php esc_html_e('Remove','education-pro'); ?></a> <span class="meta-sep"> | </span> <a class="item-cancel submitcancel" id="cancel-<?php echo esc_attr( $item_id ); ?>" href="<?php echo esc_url( add_query_arg( array('edit-menu-item' => $item_id, 'cancel' => time()), remove_query_arg( $removed_args, admin_url( 'nav-menus.php' ) ) ) );
                    ?>#menu-item-settings-<?php echo esc_attr( $item_id ); ?>"><?php esc_html_e('Cancel','education-pro'); ?></a>
            </div>
